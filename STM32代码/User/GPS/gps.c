...
float wgs84_to_bd09_lng(float lng, float lat)
{   
	float z,theta,bd_lng,mglng,mglat;
    ...
    dlng = (dlng * 180.0) / (A / sqrt(magic) * cos(radlat) * PI);
...
float wgs84_to_bd09_lat(float lng, float lat)
{
	float z,theta,bd_lat,mglng,mglat;
	float dlng = transformlng(lng - 105.0, lat - 35.0);
	float dlat = transformlat(lng - 105.0, lat - 35.0);
    float radlat = lat / 180.0 * PI;
    float magic = sin(radlat);
...
void NMEA_GNRMC_Analysis(nmea_msg *gpsx,u8 *buf)
{
	...
	gpsx->latitude=wgs84_to_bd09_lat(gpsx->longitude,gpsx->latitude);
	gpsx->longitude=wgs84_to_bd09_lng(gpsx->longitude,gpsx->latitude);
...
{			 
	u16 len=0,i;
	u8 rval=0;
	while((USART_RX_STA3&0X8000)==0 && len<100)//等待接收到应答   
	{
		len++;
		delay_ms(5);
	}		 
	if(len<100)   	//超时错误.
	{
		len=USART_RX_STA3&0X7FFF;	//此次接收到的数据长度 
		for(i=0;i<len;i++)
		{
			if(USART_RX_BUF3[i]==0X83)break;
			else if(USART_RX_BUF3[i]==0X84)
			{
				rval=3;
				break;
			}
...
{
	SkyTra_baudrate *cfg_prt=(SkyTra_baudrate *)USART_TX_BUF3;
...
{
	u32 temp=width;
	SkyTra_pps_width *cfg_tp=(SkyTra_pps_width *)USART_TX_BUF3;
...
{
	SkyTra_PosRate *cfg_rate=(SkyTra_PosRate *)USART_TX_BUF3;
